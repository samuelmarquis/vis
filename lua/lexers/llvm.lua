...
-- Identifiers.
local named_tail = (S('-.$_') + lexer.alpha) * (S('-$._') + lexer.alnum)^0
local local_named = '%' * ident_tail
local global_named = '@' * ident_tail

local local_unnamed = '%' * lexer.dec_num
local global_unnamed = '@' * lexer.dec_num

lex:add_rule('identifier', lex:tag(lexer.IDENTIFIER,
						 local_named + local_unnamed + global_named + global_unnamed))
